...
public class Container<T> {//类泛型 
    ...
    private final ReentrantLock lock=new ReentrantLock(true);// true表示使用公平锁，默认是非公平锁
    private final Condition producerCondition=lock.newCondition();//通讯条件
    private final Condition consumerCondition=lock.newCondition();//通讯条件
    /**
     * 生产者线程通过put方法向容器放数据
     * 数据永远放在size位置
     * 说明：实例方法内部的this永远指向
