...
public class JdkServiceProxyUtil {
	...
	/**
	 * 通过此方法为目标对象创建一个代理对象
	 * @return 返回指定接口的代理类的实例
	 */
	public static Object newProxy(Object target){
		//JDK中的动态代理(设计模式中的一种)为目标对象产生代理对象
		return Proxy.newProxyInstance(
				target.getClass().getClassLoader(),//claslaoder代理对象要使用类加载器
				target.getClass().getInterfaces(),//代理对象实现类要实现的接口
				new ServiceProxyHandler(target));//处理器invocationHandler
	}
}
